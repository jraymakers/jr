...
function createRun(jobDefs, jobNames, opts) {
  ...
  while (queue.length > 0) {
    ...
    if (!jobDef) {
      // If an undefined job name looks like an option, just ignore it instead of erroring out.
      // This allows such options to be passed to jobs files.
      if (currentJobName.startsWith('-')) {
        continue;
      }
