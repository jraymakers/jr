...
function makeAsyncTasks(jobNames, jobs) {
  ...
  while (queue.length > 0) {
    ...
    if (!asyncTasks[jobName]) {
      ...
      if (!action) {
        return { error: new Error('Job "' + jobName + '" is missing an "action" parameter.') };
      }
...
function loadJobsFile(jobsFile, cb) {
  jobsFile = jobsFile || path.join(process.cwd(), 'jobs.js');
  fs.exists(jobsFile, function (exists) {
    if (exists) {
      try {
        ...
        cb(new Error('Jobs file "' + jobsFilePath + '" could not be loaded: ' + ex));
      }
    } else {
      cb(new Error('Jobs file "' + jobsFilePath + '" was not found.'));
    }
  });
}

function runJobsFromFile(jobNames, jobsFile, cb) {
  loadJobsFile(jobsFile, function (err, jobs) {
    if (err) {
      cb(err);
    } else {
